$(document).ready(function(){
	...
	GBC.Accordion = function(){
		...
		function showAccordionContent($elem){
			...
			TweenMax.to($elem, 0.5, {height: elemHeight, ease:Expo.easeOut, force3D: true});
		}

		function hideAccordionContent($elem){
			TweenMax.to($elem, 0.5, {height: 0, ease:Expo.easeOut, force3D: true});
		}
	...
	GBC.TruncatedBox = (function(options){
		...
		var init = function () {
			...
			self.options.closeBtn.on('click', function(){
				...
				TweenMax.to(elem, 1, {height: realHeight, ease:Expo.easeInOut});
				TweenMax.to($(this), 0.5, {autoAlpha: 0, yPercent: "200%"});
				TweenMax.to(elem.find('.transparent-overlay'), 1, {delay: 0.5, autoAlpha: 0, yPercent: "200%"});
			});
		};
	...
	GBC.Team = (function() {
		...
		var hideDetails = function (elem) {
			...
			TweenMax.to(icon, 0.7, {autoAlpha: 0, xPercent: "200%", ease:Expo.easeOut, force3D: true});
			TweenMax.to(designation, 0.7, {autoAlpha: 0, yPercent: "200%", force3D: true});				
			TweenMax.to(name, 1, {autoAlpha: 0, yPercent: "200%", ease:Expo.easeOut, force3D: true});
